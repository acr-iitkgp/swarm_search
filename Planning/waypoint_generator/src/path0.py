...
def listener():
    global pub1
    rospy.init_node('path0', anonymous=True)
    pos_sub = rospy.Subscriber('/drone0/chatter', point_list, posCallback)
    current_pos = rospy.Subscriber("/drone0/mavros/global_position/global",NavSatFix, current_callback)
    flags = rospy.Subscriber("/drone0/flags",Twist,flag_callback)
