...
def listener():
    global pub1
    rospy.init_node('path3', anonymous=True)
    pos_sub = rospy.Subscriber('/drone3/chatter', point_list, posCallback)
    current_pos = rospy.Subscriber("/drone3/mavros/global_position/global",NavSatFix, current_callback)
    flags = rospy.Subscriber("/drone3/flags",Twist,flag_callback)
    pub1 = rospy.Publisher('/drone3/ROI_flow', point_list, queue_size=5)
    rospy.spin()
